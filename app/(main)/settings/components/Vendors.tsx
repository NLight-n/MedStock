...
export default function Vendors() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await fetch(`/api/settings/vendors/${editingId}`, {
          method: 'PUT',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 shadow sm:rounded-lg">
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          {vendors.map((vendor) => (
            <li key={vendor.id} className="px-6 py-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 flex items-center justify-between">
              <div>
                <div className="font-medium">{vendor.name}</div>
                <div className="text-gray-500 dark:text-gray-400">{vendor.description}</div>
                <div className="text-gray-500 dark:text-gray-400">{vendor.website}</div>
                {vendor.contactPerson && <div className="text-gray-500 dark:text-gray-400">Contact: {vendor.contactPerson}</div>}
                {vendor.contactEmail && <div className="text-gray-500 dark:text-gray-400">Email: {vendor.contactEmail}</div>}
                {vendor.contactPhone && <div className="text-gray-500 dark:text-gray-400">Phone: {vendor.contactPhone}</div>}
                {vendor.address && <div className="text-gray-500 dark:text-gray-400">Address: {vendor.address}</div>}
              </div>
              <div className="flex gap-2 ml-auto">
                <button className="text-blue-600 hover:underline" onClick={() => handleEdit(vendor)}>Edit</button>
                <button className="text-red-600 hover:underline" onClick={() => handleDelete(vendor.id)}>Delete</button>
              </div>
            </li>
          ))}
        </ul>
